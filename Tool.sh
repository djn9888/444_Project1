...
#kill all APM and ifstat prcocesses
cleanup(){
...
#start all APM prcocesses in headless mode
startAPMs(){
   n=1
   while [ $n -le 6 ]
   do
...
writeProcessMetrics(){
   ...
   #set array for number of APM prcocesses running
   #then loop through them with ps aux to gather process data
   n=(1 2 3 4 5 6)
      y="$times,"
      for apm in "${n[@]}"
      do
